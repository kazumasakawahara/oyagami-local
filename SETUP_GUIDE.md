...
### ffmpeg のインストール（音声文字起こし用）

面談記録の音声アップロード機能（Whisper）に必要です。
...
💡 **簡単な起動方法**: `./scripts/setup.sh` で全サービスを一括起動できます。詳細は `./scripts/setup.sh --help` を参照してください。
...
自然文のテキストを入力するか、Word/Excel/PDF/テキストファイルをアップロードすると、LLMが構造化されたデータに変換します。

1. テキストエリアに支援情報を入力する、またはファイル（docx/xlsx/pdf/txt）をアップロードする
...
キーワードではなく「意味の近さ」で支援記録やケア指示を検索します。ベクトル類似度を使うため、表現が異なる記録でも関連する情報を発見できます。

### Ecomap（エコマップ）

クライアントを選択してテンプレートを選ぶと、支援ネットワーク全体をグラフで表示します。Neo4j Browser風のダーク背景で、支援者・家族・医療機関などの関係を視覚的に確認できます。

### Meetings（面談記録）

音声ファイル（mp3/wav/m4aなど）をアップロードすると、Whisperが自動で文字起こしを行い、Neo4jに保存します。保存後はセマンティック検索の対象になります。
